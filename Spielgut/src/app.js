...
import { getNewProductsDia, getUsedProductsDia } from "./model.js";
...
async function handleRegister(request) {
  ...
  try {
    await registerUser(username, email, password, straße, hausnummer, stadt, plz);
    return new Response("", {
      status: 302,
      headers: { "Location": "/login" },
    });
  } catch (error) {
    return new Response(await render("register.html", { error: "Registrierung fehlgeschlagen" }), {
      status: 400,
      headers: { "content-type": "text/html" },
    });
  }
...
async function serveAssetFile(path) {
  ...
  if (path.startsWith("/assets/")) {
    ...
    // Für Produktbilder
    filePath = `./assets/Produktbilder${path}`;
  }
...
const handler = async (request) => {
  ...
  switch (path) {
    case "/":
      const newProductsDia = await getNewProductsDia();
      const usedProductsDia = await getUsedProductsDia();
      content = await render("index.html", { user, newProductsDia, usedProductsDia });
      break;
    case "/new-products":
      content = await render("new-products.html", { user });
      break;
    case "/used-products":
